...
/**
 * This models supports retrieving lists of forsales.
 *
 * @package		Joomla.Site
...
class ForsalesModelForsales extends JModelList
{
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.$this->getState('filter.forsale_id');
	...
	/**
	 * Get the master query for retrieving a list of forsales subject to the model state.
	 *
...
//echo nl2br(str_replace('#__','foo_',$query));
	...
	/**
	 * Method to get a list of forsales.
	 *
	...
	public function getItems()
	{
		...
		$globalParams = JComponentHelper::getParams('com_forsales', true);

		// Convert the parameter fields into objects.
		foreach ($items as $item)
		{
			$forsaleParams = new JRegistry;
			$forsaleParams->loadString($item->parameters);

			$item->layout = $forsaleParams->get('layout');

			$item->parameters = clone $this->getState('params');
			$item->parameters->merge($forsaleParams);

			// Compute the asset access permissions.
			// Technically guest could edit an forsale, but lets not check that to improve performance a little.
			if ($guest) {
            } else {
				$asset	= 'com_forsales.forsale.'.$item->id;
			...
			if ($access) {
				// If the access filter has been set, we already have only the forsales this user can view.
