...
class ForsalesModelForsale extends JModelItem
{
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('forsale.id');
		...
		if (!isset($this->_item[$pk])) {
			...
			try {
				...
				if (empty($data)) {
					return JError::raiseError(404,JText::_('COM_FORSALES_ERROR_FORSALE_NOT_FOUND'));
				}
				...
				if ($user->get('guest')) {
					...
					$userId	= $user->get('id');
					$asset	= 'com_forsales.forsale.'.$data->id;
	...
	function getItemQuery($id)
	{
		...
		$forsaleId = $id;
		if ((int) ($forsaleId) > 0) {
			$query->where('a.id = '.(int) $forsaleId);
		}
