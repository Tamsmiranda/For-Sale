...
/**
 * HTML Article View class for the Forsales component
 *
 * @package		Joomla.Site
 * @subpackage	com_forsales
 * @since		1.5
 */
class ForsalesViewForm extends JView
{
	protected $form;
	protected $item;
	...
	public function display($tpl = null)
	{
		...
		if (empty($this->item->id)) {
			$authorised = $user->authorise('core.create', 'com_forsales') || (count($user->getAuthorisedCategories('com_forsales', 'core.create')));
		} else {
		...
		if ($this->parameters->get('enable_category') == 1) {
			...
			$category = JCategories::getInstance('Forsales')->get($this->parameters->get('catid', 1));
