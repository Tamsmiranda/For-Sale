...
class ForsalesControllerForsale extends JControllerForm
{
	 ...
	 *
	 * @return	boolean	True if the forsale can be added, false if not.
	 * @since	1.6
	 */
	public function add()
	{
	...
	protected function allowAdd($data = array())
	{
		...
		if ($categoryId) {
			...
			$allow	= $user->authorise('core.create', 'com_forsales.category.'.$categoryId);
		}
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		$user		= JFactory::getUser();
		$userId		= $user->get('id');
		$asset		= 'com_forsales.forsale.'.$recordId;
...
//		}
	...
	public function save($key = null, $urlVar = 'a_id')
	{
		...
		require_once JPATH_ADMINISTRATOR.'/components/com_forsales/helpers/forsales.php';
	...
	function vote()
	{
		...
		if ( $user_rating > -1 ) {
			...
			if ($model->storeVote($id, $user_rating)) {
				$this->setRedirect($url, JText::_('COM_FORSALES_FORSALE_VOTE_SUCCESS'));
			} else {
				$this->setRedirect($url, JText::_('COM_FORSALES_FORSALE_VOTE_FAILURE'));
			}
		}
	}
}